...
export const DEFAULT_INTENT_TOOL_MAP: IntentToolMap = {
  product_search: [{ tool_id: 'product_search', input: {} }],
  price_query: [{ tool_id: 'price_check', input: {} }],
  product_search_with_price: [
    { tool_id: 'product_search', input: {} },
    { tool_id: 'price_check', input: {} },
  ],
  order_query: [{ tool_id: 'order_query', input: {} }],
  calculate: [{ tool_id: 'calculate', input: {} }],
}

// ── Manifest ────────────────────────────────────────────────────────────────

export function createWorkManifest(overrides: Partial<AgentManifest> = {}): AgentManifest {
  return {
 ...
 * ```
 */
export class WorkAgent extends StreamAgent {
