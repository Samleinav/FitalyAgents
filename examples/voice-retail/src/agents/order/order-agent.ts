...
import type { TaskPayloadEvent, TaskResultEvent, AgentManifest, IEventBus } from 'fitalyagents'
import type { IOrderService } from './types.js'

// ── Manifest ─────────────────────────────────────────────────────────────────

export function createOrderManifest(overrides: Partial<AgentManifest> = {}): AgentManifest {
  return {
 ...
 * - `requires_human_approval: true` in manifest — handled by ApprovalQueue
 ...
 * ```
 */
export class OrderAgent extends StreamAgent {
  ...
  private buildFailure(task_id: string, session_id: string, err: unknown): TaskResultEvent {
