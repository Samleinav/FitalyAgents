...
/**
 * Simplified task router that listens for TASK_AVAILABLE
 * and dispatches directly to the appropriate agent via the bus.
 *
 * In production, this is handled by CapabilityRouter + TaskQueue.
 * For E2E tests, we wire it manually for clarity.
 */
function createSimpleRouter(bus: InMemoryBus) {
    const intentToAgent: Record<string, string> = {
        product_search: 'queue:work-agent:inbox',
        product_search_with_price: 'queue:work-agent:inbox',
        price_query: 'queue:work-agent:inbox',
        order_query: 'queue:work-agent:inbox',
    }

    return bus.subscribe('bus:TASK_AVAILABLE', (data) => {
        const event = data as {
            task_id: string
            session_id: string
            intent_id: string
        ...
        const agentChannel = intentToAgent[event.intent_id]
        ...
        const interactionPayload = {
            event: 'TASK_PAYLOAD',
            task_id: `interaction_${event.task_id}`,
            session_id: event.session_id,
            intent_id: event.intent_id,
            slots: event.slots ?? {},
            context_snapshot: {},
        ...
        // Route via lpush (queue) — NexusAgent uses brpop to receive
        void bus.lpush(agentChannel, workPayload)
        void bus.lpush('queue:interaction-agent:inbox', interactionPayload)
    })
}
